    ...
    \begin{tikzpicture}
        ...
        \only<5-18>{
            ...
            \only<7->{
                ...
                \draw[alt=<18>{draw=red, stealth-}{draw=black, -stealth}] (input) -- ($ (model.west) - (0, 0.2175) $);
            }
            \only<12->{
                \node[anchor=west, alt=<13->{text=red}{text=black}] (output) at (3, 1.25) {$\text{ladybug}$};
                \draw[
                    alt=<12>{draw=yellow, -stealth},
                    alt=<13>{draw=red, stealth-}
                ] ($ (model.south east) + (0, 1.25) $) -- (output);
            }
            \only<5-6>{
                \node[circle, draw=black, fill=uiolightblue, minimum size=0.5cm] (neuron) at ($ (model.south) - (0, 1.5) $) {};
            ...
            \foreach \j in {0,...,4} {
                \draw[draw=black, alt=<7>{draw=yellow}{}, alt=<18>{draw=red}{}, opacity=\edgeopacity] ($ (model.west) - (0, 0.2175) $) -- (n0\j);
            }

            \foreach \j in {0,...,4} {
                \foreach \k in {0,...,3} {
                    \draw[draw=black, alt=<8>{draw=yellow}{}, alt=<17>{draw=red}{}, opacity=\edgeopacity] (n0\j) -- (n1\k);
                }
            }
            \foreach \j in {0,...,3} {
                \foreach \k in {0,...,2} {
                    \draw[draw=black, alt=<9>{draw=yellow}{}, alt=<16>{draw=red}{}, opacity=\edgeopacity] (n1\j) -- (n2\k);
                }
            }
            \foreach \j in {0,...,2} {
                \foreach \k in {0,...,1} {
                    \draw[draw=black, alt=<10>{draw=yellow}{}, alt=<15>{draw=red}{}, opacity=\edgeopacity] (n2\j) -- (n3\k);
                }
            }
            \draw[draw=black, alt=<11>{draw=yellow}{}, alt=<14>{draw=red}{}, opacity=\edgeopacity] (n30) -- (n40);
            \draw[draw=black, alt=<11>{draw=yellow}{}, alt=<14>{draw=red}{}, opacity=\edgeopacity] (n31) -- (n40);
            \draw[draw=black, alt=<12>{draw=yellow}{}, alt=<13>{draw=red}{}, opacity=\edgeopacity] (n40) -- ($ (model.south east) + (0, 1.25) $);
        }
        \only<19>{
