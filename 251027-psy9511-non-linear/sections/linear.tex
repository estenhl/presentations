...
\begin{frame}{Extensions of linear models: Generalized linear models}
    \begin{tikzpicture}
        ...
        \visible<1-5>{
            \node[] at (0, -2.5) {
                $\hat{y}=\alert<4>{\beta_0+\sum\limits_{i=0}^p \beta_ix_i}$
            };
        }
        \visible<2-8,11-12>{
            \node[] at (0, 1) {
                \only<2>{\usebox{\linearbox}}%
                \only<3-4,11>{\usebox{\exponentialbox}}%
                \only<5>{\usebox{\loglinearbox}}%
                \only<6>{\usebox{\logisticbox}}%
                \only<7-8>{\usebox{\logregbox}}%
                \only<12>{\usebox{\expregbox}}%
            };
        }
        \visible<6>{
            \node[] at (0, -2.5) {
                $\log\left(\dfrac{p(y=1\ |\ X)}{1 - p(y=1\ |\ X)}\right)=\beta_0+\sum\limits_{i=0}^p \beta_ix_i$
            };
        }
        \visible<7-8>{
            \node[] at (0, -2.5) {
                $p(y=1\ |\ X)=\dfrac{e^{\left(\alert<8>{\beta_0+\sum\limits_{i=0}^p \beta_ix_i}\right)}}{1 + e^{\left(\alert<8>{\beta_0+\sum\limits_{i=0}^p \beta_ix_i}\right)}}$
            };
        }
        \visible<9>{
            \node[] at (0, -2.5) {
                $\hat{y}=f(\beta_0+\sum\limits_{i=0}^p \beta_ix_i)$
            };
        }
        \visible<10-11>{
            \node[] at (0, -2.5) {
        ...
        \visible<14>{
            \node[align=flush left, text width=10.1cm] at (0, 0) {
                ...
                Extends the regular linear model by associating the predictors to the response via a non-linear link function $f$.
                \begin{itemize}
...
\begin{frame}{Extensions of linear models: Generalized additive models}
    \begin{tikzpicture}
        ...
        \visible<1-3>{
            \node[] at (0, 1) {
                \only<1>{\usebox{\complexbox}}%
                \only<2>{\usebox{\splinebox}}%
                \only<3>{\usebox{\singlesplinebox}}
            };
        }
        ...
        \visible<12>{
            \node[align=flush left, text width=10.1cm] at (0, 0) {
                ...
                Extends the linear model by allowing non-linear functions $f_j$ (i.e. splines) to be fitted for each predictor $x_j$.
                \begin{itemize}
