    ...
    \begin{tikzpicture}
        ...
        \visible<1-6>{
            \node[anchor=north west, ampersand replacement=\&, font=\footnotesize] at (-5.25, 3.5) {
                \begin{tabular}{|c|c|c|c|c|c|}
                    \hline
                    \textbf{name}&\alert<3,5>{\textbf{year}}&\alert<5>{\textbf{cylinders}}&\alert<5>{\textbf{horsepower}}&\alert<5>{\textbf{weight}}&\alert<4>{\textbf{mpg}}\\
                    \hline
                    Chevrolet Chevelle Malibu&\alert<3,5>{1970}&\alert<5>{8}&\alert<5>{130}&\alert<5>{3504}&\alert<4>{18}\\
                    \alert<2>{Buick Skylark 320}&\alert<2,3,5>{1980}&\alert<2,5>{4}&\alert<2,5>{165}&\alert<2,5>{3693}&\alert<2,4>{15}\\
                    Plymouth Satellite&\alert<3,5>{1971}&\alert<5>{8}&\alert<5>{150}&\alert<5>{3436}&\alert<4>{18}\\
                    AMC Rebel SST&\alert<3,5>{1975}&\alert<5>{4}&\alert<5>{150}&\alert<5>{3433}&\alert<4>{16}\\
                    Ford Torino&\alert<3,5>{1978}&\alert<5>{8}&\alert<5>{140}&\alert<5>{3449}&\alert<4>{17}\\
                    \hline
                \end{tabular}
            };
            \node[anchor=south west, text width=10.5cm] at (-5.25, -3.5) {
                ...
                \begin{itemize}
                    ...
                    \item<4-> A response-variable $y$ that we want to predict
                    \item<5-> A set of predictors $X$ that we can use to predict $y$
                    \item<6-> An \textbf{assumed} relationship between $X$ and $y$ that can be described by an unknown function $f$, such that $y=f(X) + \epsilon$
                \end{itemize}
            };
        }
        \visible<7>{
        ...
        \visible<7-11>{
            \node[anchor=north west, text width=10.5cm] at (-5.25, 0) {
                ...
                \begin{itemize}
                    ...
                    \item<8-> We produce an estimate $\hat{f}$
                    \item<10-> Parametric models: $\hat{f}$ has a simple form
                    \begin{itemize}
                        \item<9-> $\hat{f}(x) = {\color{red}\beta_0} + {\color{red}\beta_1} x$
                    \end{itemize}
                    \item<11-> Non-parametric models: $\hat{f}$ relies directly on the data
                \end{itemize}
            };
        }
        \visible<13>{
        ...
        \visible<12-18>{
            \node[anchor=north west, text width=10.5cm, align=flush left] (inference) at (-5.25, 0) {
                ...
                \begin{itemize}
                    \item<13-> How does individual features relate to the response?
                    \item<14-> What is the \textit{functional form} of the relationship?
                \end{itemize}
            };
        }
        \visible<16-18>{
    ...
    \begin{tikzpicture}
        ...
        \visible<8>{
            \node[text width=10.5cm] at (0, 0) {
                ...
                \begin{itemize}
                    \item The Bayes classifier is the best classifier that can be theoretically achieved (although in practice we have to approximate it using data)
