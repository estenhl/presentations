...
\begin{frame}{The history of artificial intelligence}
    ...
    \begin{tikzpicture}
        ...
        \node[draw=black] at (5.25, -4.2) {};

        \draw[very thick, passivehistory] (-4.75, 2.75) -- (-4.55, 2.75);
        \only<1>{
            \activenode{-4.55}{Talos\\($\sim$300 BC)}{1}
            \node[draw=black, inner sep=0pt, label=below:{\tiny{Source: ChatGPT 5}}] at (0, -0.5) {
        ...
        \only<18>{
            ...
            \node[inner sep=0pt, draw=black, label=below:{\tiny{Source: DALL-E}}] (img) at (-2.25, -1) {
