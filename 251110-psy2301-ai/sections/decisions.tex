	...
	\begin{frame}[t]{Decision-making: Summary}
		...
		\visible<2>{
			\begin{itemize}
				...
				\item[\textcolor{red}-] There are neither any guarantees these patterns are ones we want to use (e.g. bias).
			\end{itemize}
		}
	\end{frame}
	...
	\begin{frame}[t]{Decision-making: Generalization} % Extrapolation
		...
		\begin{itemize}
			...
			\item <3-> While machine learning models are trained on a specific dataset (commonly referred to as the training set), they are almost always evaluated on a different dataset (often called the test set).
		\end{itemize}
	...
	\begin{frame}[t]{Decision-making: Biases}
		...
		\only<2-5>{
			...
			\begin{itemize}
				...
				\item <4->Both COMPAS and humans were biased against black offenders, even when ethnicity was not included in the data.
		...
		\only<4-5>{
			...
			\begin{tikzpicture}
				...
				\node[font=\tiny\selectfont] at (0, -4.55) {
					\href{https://arxiv.org/abs/2302.02083}{Theory of Mind Might Have Spontaneously Emerged in Large Language Models}, Kosinski, M., \textit{PNAS}, 2024.
				};
			\end{tikzpicture}
		}
	\end{frame}
	...
	\begin{frame}[t]{Decision-making: Creativity}
		...
		\only<3>{
			...
			\vspace{0.2cm}
			\begin{tikzpicture}
				\node[label=below:\tiny{Imagen: A cute corgi lives in a house made out of sushi}, inner sep=0pt, draw=black] {
	...
	\begin{frame}[t]{Decision-making: Wisdom}
		\textbf{Are AIs wise?}
		\begin{itemize}
			\item "... machines don’t have common sense" - Korteling et al., 2021
			\item "... the expertise in the domain of fundamental life pragmatics, such as life planning or life review. It requires a rich factual knowledge about life matters, rich procedural knowledge about life problems, knowledge of different life contexts and values or priorities, and knowledge about the unpredictability of life." - Reasoning, Judging, Deciding: The Science of Thinking, Ch. 15 (adopted from Birren and Svensson, attributed to Baltes and Smith)
	...
	\begin{frame}[t]{Decision-making: Summary}
		...
		\begin{itemize}
			...
			\item[\textcolor{red}{$\rightarrow$}] <2> \textcolor{red}{What is wisdom, creativity?}
		\end{itemize}
	\end{frame}