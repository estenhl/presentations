...
\newcommand{\modelchange}[1]{
    \begin{tikzpicture}
        ...
        \ifnum#1=0
            \node[anchor=west, font=\scriptsize, text=blue] at (rel axis cs: 0.25, 0.93) {
                $\hat{f}(x)=39.93-0.07x$
            };
            \node[anchor=west, font=\scriptsize, text=red] at (rel axis cs: 0.25, 0.83) {
                $\hat{f}(x)=39.93-0.15x$
            };
            \node[anchor=west, font=\scriptsize, text=cyan] at (rel axis cs: 0.25, 0.73) {
                $\hat{f}(x)=39.93-0.21x$
            };
        \fi
    ...
    \begin{tikzpicture}
        ...
        \only<4>{
            \node[font=\LARGE] at (0, -1) {
                $MSE=\dfrac{1}{n}\sum\limits_{i=0}^{n}(y_i-\alert<4>{\hat{y}}_i)^2$
            };
        }
        \only<5>{
            \node[font=\LARGE] at (0, -0.5) {
                $MSE=\dfrac{1}{n}\sum\limits_{i=0}^{n}(y_i-\beta_0+\beta_1x_i)^2$
            };
        }
        \only<6>{
            \node[] at (-2.675, 0) {
