...
\begin{document}
    ...
    \begin{frame}{Overview}
        \begin{enumerate}
            ...
            \item Outlook: Emerging challenges and trends
        \end{enumerate}
    \end{frame}
    ...
    \begin{frame}[t]{Background}
        ...
        \begin{tikzpicture}
            ...
            \only<2-4>{
                ...
                \onslide<4->{
                    \node[inner sep=0pt, draw=black] at (2.2, -4) {
                        \includegraphics[width=4.3cm]{data/fmri_signal_brain.jpeg}
                    };

                    \node[font=\tiny, anchor=south,align=center, inner sep=1pt] (ms-citation) at (0, -7.65) {
            ...
            \onslide<3-4>{
                \node[inner sep=0pt, label=below:{\labelsize{Bert from FreeSurfer 7.3}}] at (-2.4, -4) {
                    \includegraphics[width=3.8cm]{data/bert_sagittal_enhanced.png}
                };
            }
            \onslide<6-9>{
                \node[inner sep=0pt] at (-3, -4) {
                    \includegraphics[width=2.5cm]{data/bert_sagittal_enhanced.png}
                };
            ...
            \only<5-9>{
                ...
                \only<8>{
                    \node[inner sep=0pt, label=above:{\labelsize{Front}}] at (0, -4) {
                        \includegraphics[width=2.5cm]{data/bert_z_enhanced.png}
                    };
                    \node[inner sep=0pt, label=above:{\labelsize{Above}}] at (3, -4) {
                        \includegraphics[width=2.5cm]{data/bert_y_enhanced.png}
                    };
                }
                \only<9>{
                    \node[] at (2, -4.2) {
    ...
    \begin{frame}[t]{Structural MRI modalities}
        \only<1-13>{
            \begin{tikzpicture}
        ...
        \only<14>{
            \vfill
            Structural MRI beyond T1-weighted: Picturing different, stable, properties of tissue.
            \begin{itemize}
                \item[\textbullet] {\small Most prevalent in studies classifying MS and PD, yielding high accuracies ($\sim$90\%).}
                \item[\textbullet] {\small T2 intimately linked with MS due to its efficacy at exposing characteristic lesions.}
                \item[\textbullet] {\small Machine learning models trained on T1 and T2-weighted scans shown by Storelli et al. to predict prognosis in MS better than expert humans.}
            \end{itemize}
            \vfill
        }
    \end{frame}
    ...
    \begin{frame}[t]{Diffusion MRI}
        \only<1-13>{
            \begin{tikzpicture}
                ...
                \only<1-9>{
                    ...
                    \only<5>{
                        \node[inner sep=0pt, draw=black] at (6.5, -3.5) {
                            \includegraphics[width=5.5cm]{data/axon.jpeg}
                        };
                        \node[font=\tiny, anchor=south,align=center, inner sep=1pt] (ms-citation) at (5, -7.65) {
        ...
        \only<14>{
            \vfill
            Diffusion MRI: Depicting white matter tracts and their integrity
            \begin{itemize}
                \item[\textbullet] {\small Relatively few prediction studies found, most studies appear to focus on deriving biological insights.}
                \item[\textbullet] {\small Most studies for mental disorders, specifically SCZ and MDD, with high mean accuracies ($\sim$85\%) but a large spread (60-100\%).}
                \item[\textbullet] {\small Used by Saglam et al. to differentially diagnose SCZ and BP with 80\% accuracy, approaching the threshold for clinical utility.}
            \end{itemize}
            \vfill
        }
    \end{frame}
    ...
    \begin{frame}[t]{Functional Magnetic Resonance Imaging (fMRI)}
        ...
        \only<12>{
            \vfill
            Functional MRI: An indirect measure of brain activity
            \begin{itemize}
                \item[\textbullet] {\small A large number of predictive studies found, mostly for mental disorders.}
                \item[\textbullet] {\small Generally high mean accuracies (80-85\%) but a large spread (50-100\%).}
                \item[\textbullet] {\small Sundermann et al. struggled to find signal in a sample of diverse patients, but saw predictive efficacy when restricted to those with the most severe symptoms.}
            \end{itemize}
            \vfill
        }
    \end{frame}
    ...
    \begin{frame}[t]{Molecular imaging}
        ...
        \only<22>{
            \vfill
            Molecular imaging: Detects the presence of specific molecules
            \begin{itemize}
                \item[\textbullet] {\small Used in studies classifying PD and DEM with good results (accuracies >90\%), although (potentially surprisingly) not substantially better than other modalities.}
                \item[\textbullet] {\small PET is a natural choice for detecting AD-related pathology, both A$\beta$ plaues and TAU tangles.}
                \item[\textbullet] {\small SPECT useful to characterize degeneration of dopamine-producing cells in PD}
                \item[\textbullet] {\small Useful for differentially diagnosing neurological disorders underlying DEM, as shown by Rogeau et al.}
            \end{itemize}
            \vfill
        }
    \end{frame}

    \section{Emerging challenges and trends}
        ...
        \begin{tikzpicture}
            \begin{groupplot}[
                ...
                \nextgroupplot[
                    ...
                    \addplot[black, thick] coordinates {
                        (0, 92.5003) (1600, 92.5003+1600*-0.0009)
                    };
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \only<1>{
                    ...
                    \node[anchor=south east] (dllabel) at (axis cs: 2007, 52) {\small{Deep learning}};
                    \draw[dlcolor, very thick] (dllabel.west) -- ($ (dllabel.west) - (1.5, 0) $);
                    \node[anchor=south west] (mllabel) at ($ (dllabel.north west) - (0, 15) $) {\small{Machine learning}};
