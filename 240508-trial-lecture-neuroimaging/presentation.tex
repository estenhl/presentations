...
\begin{document}
    ...
    \newcommand{\casecontrol}[1]{
        \begin{tikzpicture}
            \node[draw=black] at (-0.5, -0.5) {};
            \node[draw=black] at (7.8, -6) {};
            % Randomly placed red stickmen
    ...
    \begin{frame}[t]{Other structural MRI modalities}
        \only<1-13>{
            \begin{tikzpicture}
                ...
                \only<11>{
                    \node[circle, draw=red, thick, fill=none, inner sep=5pt] at (8.025, -2.28) {};
                    \node[font=\tiny, anchor=south,align=center, inner sep=1pt] (ms-citation) at (5, -7.65) {
                        Eitel, F., Soehler, E., Bellmann-Strobl, J., Brandt, A. U., Ruprecht, K., Giess, R. M., ... \& Ritter, K. (2019). Uncovering convolutional\\neural network decisions for diagnosing multiple sclerosis on conventional MRI using layer-wise relevance propagation.\\NeuroImage: Clinical, 24, 102003
                    };
                }
                \only<12-13>{
                    \node[font=\tiny, anchor=south,align=center, inner sep=1pt] (ms-citation) at (5, -7.65) {
    ...
    \newsavebox{\dmribp}
    \sbox{\dmribp}{%
        \disordermodalityplot{BP}{dMRI}{dmri_studies.csv}{79.32}{150}{bp}{160}
    }
    ...
    \begin{frame}[t]{Functional Magnetic Resonance Imaging (fMRI)}
        \only<1-8>{
            \begin{tikzpicture}
                ...
                \only<7-8>{
                    \node[font=\tiny, anchor=south,align=center, inner sep=1pt] (ms-citation) at (5, -7.65) {
                        Duan, J., Li, Y., Zhang, X., Dong, S., Zhao, P., Liu, J., ... \& Wang, F. (2023). Predicting treatment response in adolescents\\and young adults with major depressive episodes from fMRI using graph isomorphism network. NeuroImage: Clinical, 40, 103534
                    };
                }

                \only<7>{
                    \node[inner sep=0pt, draw=black] at (5, -3.2) {
                        \includegraphics[width=9cm]{data/mdd_model.jpg}
                    };
                }
                \only<8>{
                    \node[inner sep=0pt, draw=black] at (5, -3.2) {
                        \includegraphics[width=7cm]{data/mdd_results.png}
                    };
                }
            \end{tikzpicture}
        }
