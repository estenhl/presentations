...
%     \centering
%     Will transfer learning from the pretrained mulit-task model result in better predictive performance in downstream prediction tasks relying on small training datasets?
% \end{frame}
...
%             \node[font=\footnotesize] (error2) at (6, -0.6) {Error};
...
\begin{frame}{Results: Brain age}
    If you want to accurately predict anything in new samples:
    \begin{itemize}
        \item If you are using a CNN, you should \textit{always}* use a pretrained model, and a multi-task model outperforms a single-task model even when the pretraining task is closely related to the downstream task
        \item However, apparently, you can get quite some way with simple linear models
    \end{itemize}
\end{frame}