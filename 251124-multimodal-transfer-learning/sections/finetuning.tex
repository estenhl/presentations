...
\begin{frame}{Results: Brain age}
    If you want to accurately predict brain age in new samples with a CNN:
    \begin{itemize}
        ...
        \begin{axis}[
            ...
            \node[draw=black, fill=white, anchor=south east, text width=4cm, inner sep=2pt, font=\footnotesize] at (rel axis cs: 0.98, 0.02) {
                ...
                \begin{tabular}{cl}
                    ...
                    \visible<2->{\legendtrace{multi-frozen}{\multifrozenmarker}} & \visible<2->{Frozen multi-task}\\
                    \visible<3->{\legendtrace{multi-finetuned}{\multifinetunedmarker}} & \visible<3->{Finetuned multi-task}\\
                    \visible<4->{\legendtrace{linear}{\linearmarker}} & \visible<4->{Linear}\\
                \end{tabular}
...
\begin{frame}{Results: Brain age}
    \begin{tikzpicture}
        ...
        \node[text width=13cm] at (0, 0) {
            ...
            \begin{itemize}
                \item If you are using a CNN, you should \textit{always}\textsuperscript{2} use a pretrained model.
                \begin{itemize}
                    \item The multi-task model outperforms a single-task model even when the pretraining task is closely related to the downstream task
                \end{itemize}
