...
\begin{frame}{What does neural networks do?}
    \begin{tikzpicture}
        ...
        \visible<18>{
            \draw[|-|, thick, red] ($ (firstfeatures.north west) + (0.1, 0) $) -- ($ (firstfeatures.south west) + (0.1, 0) $);
            \node[circle, thick, draw=red, inner sep=2.5pt] (neuron) at ($ (firstfeatures.north west) - (0.18, -1.06) $) {};
            \draw[stealth-stealth, dashed, red] (neuron) -- ($ (firstfeatures.north west) + (0.1, 0) $);
        }
        \visible<20>{
