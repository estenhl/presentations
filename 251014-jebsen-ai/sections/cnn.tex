\newcommand{\mriside}[5][input]{
    \def\mridepth{0.75}

    \node[inner sep=0pt] (#1) at (#2, #3) {
        \includegraphics[height=#4, width=#4]{#5}
    };

    \draw[fill=black] (#1.north west) --
        ($ (#1.north west) + (0.5 * \mridepth, 0.5 * \mridepth) $) --
        ($ (#1.north east) + (0.5 * \mridepth, 0.5 * \mridepth) $) --
        (#1.north east) -- cycle;
    \draw[fill=black] (#1.north east) --
        ($ (#1.north east) + (0.5 * \mridepth, 0.5 * \mridepth) $) --
        ($ (#1.south east) + (0.5 * \mridepth, 0.5 * \mridepth) $) --
        (#1.south east) -- cycle;
    \draw[] (#1.north west) --
        ($ (#1.north west) - (0.5 * \mridepth, 0.5 * \mridepth) $) --
        ($ (#1.south west) - (0.5 * \mridepth, 0.5 * \mridepth) $) --
        (#1.south west) -- cycle;
    \draw[] (#1.north east) --
        ($ (#1.north east) - (0.5 * \mridepth, 0.5 * \mridepth) $) --
        ($ (#1.south east) - (0.5 * \mridepth, 0.5 * \mridepth) $) --
        (#1.south east) -- cycle;
    \draw[] ($ (#1.north west) - (0.5 * \mridepth, 0.5 * \mridepth) $) --
        ($ (#1.north east) - (0.5 * \mridepth, 0.5 * \mridepth) $);
    \draw[] ($ (#1.south west) - (0.5 * \mridepth, 0.5 * \mridepth) $) --
        ($ (#1.south east) - (0.5 * \mridepth, 0.5 * \mridepth) $);
}
...
\newcommand{\cnn}[6]{
    ...
    \node[anchor=south, text depth=0, font=\footnotesize\selectfont] at (#1 + 2.675, #2 + 1.43) {
        \textbf{Konvolusjonelt nevralt nettverk}
    };
}
\newcommand{\lrp}[4]{
    \def\xmin{#1}
...
\newcommand{\unet}[6]{
    ...
    \convlayer{#1 - 0.06 + 0.4}{#2 + 2.5 * #4}{#3}{#4}{12}{3}{\modelcolour}
    \cnnarrow{(#1 + 0.91, #2)}{(#1+2.2, #2)}{black}

    \convlayer{#1 + 1.1 + 0.4}{#2 + 1.5 * #4}{#3}{#4}{8}{5}{\modelcolour}
    \cnnarrow{(#1 + 2.02, #2)}{(#1+2.8, #2)}{black}

    \convlayer{#1 + 2.06 + 0.4}{#2 + 0.5 * #4}{#3}{#4}{4}{7}{\modelcolour}
    \cnnarrow{(#1 + 2.93, #2)}{(#1+3.5, #2)}{black}

    \convlayer{#1 + 2.82 + 0.4}{#2 + 0}{#3}{#4}{2}{9}{\modelcolour}
    \cnnarrow{(#1 + 3.72, #2)}{(#1+4.4, #2)}{black}

    \convlayer{#1 + 3.53 + 0.4}{#2 + 0.5 * #4}{#3}{#4}{4}{7}{\modelcolour}
    \cnnarrow{(#1 + 4.4, #2)}{(#1+5.2, #2)}{black}

    \convlayer{#1 + 4.29 + 0.4}{#2 + 1.5 * #4}{#3}{#4}{8}{5}{\modelcolour}
    \cnnarrow{(#1 + 5.21, #2)}{(#1+6, #2)}{black}

    \convlayer{#1 + 5.25 + 0.4}{#2 + 2.5 * #4}{#3}{#4}{12}{3}{\modelcolour}

    \draw[thick, dashed] (#1 + 0.22, #2 + 1.43) --
                        (#1 + 6.75, #2 + 1.43) --
                        (#1 + 6.75, #2 - 1.42) --
                        (#1 + 0.22, #2 - 1.42) -- cycle;
    \node[anchor=south, text depth=0, font=\footnotesize\selectfont] at (#1 + 3.485, #2 + 1.43) {
        \textbf{U-net}
    };
}